...
def llm_call2(user_prompt):
    ...
    completion = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "Your job is to fix this ode for manim 0.18.0 and make it work perfectly. Ur response will be parsed as raw code so do not say anything else. make sure ur render is the same function as ur first scene. Also, Create is capitalizwed in manim 0.18.0"},
            {"role": "user", "content": user_prompt}
        ]
    )
...
def main():
    while True:
        ...
        manim_output, class_name = llm_call2(llm_call(actions_list, math_prompt))
    ...
    # Assuming output_maker() function takes two arguments and returns file_path and output_folder
        ...
        video_file = os.path.join('media', 'videos', '1080p60', f'{class_name}.mp4')
